...
public:
	...
	*/
	...
	void loadAnimation(tinygltf::Model &input)
	{
		...
		skeleton.fkMatrices.resize(input.nodes.size(), glm::mat4(1.0f));
	...
	void updateAnimation(float deltaTime)
	{
		...
		updateFKmatrices(nodes[0],glm::mat4(1.0f));
	...
	*/
	...
	void drawNode(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, VulkanglTFModel::Node *node, bool hasSkeleton)
	{
		...
		for (auto &child : node->children)
		{
			drawNode(commandBuffer, pipelineLayout, child, hasSkeleton);
		}
	}

	// Draw the glTF scene starting at the top-level-nodes
	void draw(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout, bool hasSkeleton)
	{
		...
		for (auto &node : nodes)
		{
			drawNode(commandBuffer, pipelineLayout, node, hasSkeleton);
		}
	}
};
...
public:
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			glTFModel.draw(drawCmdBuffers[i], pipelineLayout,true);
	...
	void setupDescriptors()
	{
		...
		*/
		...
		std::vector<VkDescriptorPoolSize> poolSizes = {
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 4),//有待商榷
			// One combined image sampler per model image/texture
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<uint32_t>(glTFModel.materials.size())+3),//3从IBL来的
			//ssbo for skeleton matrix
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1),
		};
		// One set for matrices and one per model image/texture
		const uint32_t maxSetCount = static_cast<uint32_t>(glTFModel.materials.size()) + 2 + 3;
		...
		setLayoutBindings = {vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, 0)};
		...
		std::array<VkDescriptorSetLayout, 3> setLayouts = {descriptorSetLayouts.matrices, descriptorSetLayouts.textures,descriptorSetLayouts.fkMatrix,};
		...
		std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &textures.irradianceCube.descriptor),
			vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2, &textures.lutBrdf.descriptor),
			vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 3, &textures.prefilteredCube.descriptor),
		};
		...
		writeDescriptorSets = {
			vks::initializers::writeDescriptorSet(glTFModel.skeleton.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 0, &glTFModel.skeleton.ssbo.descriptor)
		};
		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
	...
	void generateIrradianceCube()
	{
		...
		pipelineCI.pVertexInputState = glTFSkybox.getPipelineVertexInputState({ VulkanglTFModel::VertexComponent::Position, VulkanglTFModel::VertexComponent::Normal, VulkanglTFModel::VertexComponent::UV });
		...
		for (uint32_t m = 0; m < numMips; m++) {
			for (uint32_t f = 0; f < 6; f++) {
				...
				glTFSkybox.draw(cmdBuf,pipelinelayout,false);
